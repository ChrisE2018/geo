...
public class GeoControls extends JPanel implements ActionListener
{
    ...
    {
        final File currentDir = new File ("data/").getAbsoluteFile ();
        final File file = fu.getReadFile (geo, "Select Geometry File", currentDir, ".csv");
        if (file != null)
        {
            readXml (file);
        }
    ...
    private GeoItem marshall (GeoPlane plane, Element element)
    {
        GeoItem result = null;
        final String tag = element.getTagName ();
        if (tag.equals ("GeoLine"))
        {
            final int rgb = xu.getInteger (element, "color", 0);
            final Color color = new Color (rgb);
            final Point2D.Double from = new Point2D.Double (0, 0);
            final Point2D.Double to = new Point2D.Double (0, 0);
            final GeoLine parent = new GeoLine (plane, color, from, to);
            final Map<String, GeoItem> children = marshallChildren (parent, element);
            result = parent;
        }
        else
        {
    ...
    private Map<String, GeoItem> marshallChildren (GeoLine parent, Element element)
    {
        final Map<String, GeoItem> result = new HashMap<> ();
        final List<Element> children = xu.getChildren (element);
